...
## 未実装・要相談
- 発注リストの一括インポート: 実際にLINEで共有される発送リスト形式
  （【当日分】/【宅急便分】見出し、🚚 M/D発送リスト、発送先ブロックなど）に対応した
  専用パーサー（src/lib/shippingList.js）を実装済み。発注一覧タブの「一括貼り付けで追加」
  から貼り付け→解析→プレビュー（含める/除外・行編集可）→登録の流れで使う。
  既知の制約: 品名に含まれる自由形式のサイズ表記（例: "400gup"）は品名から完全には分離して
  いないため、必要ならプレビュー画面で手動編集する。
